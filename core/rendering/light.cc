...
void DirectionalLight::calculate_csm_lightspace(
    ...
    f32 near = n0;
    /* camera w u v*/
    Vec3 cam_w = -cam_front;
    Vec3 cam_u = cam->up.cross(cam_w).norm();
    Vec3 cam_v = cam_w.cross(cam_u).norm();
    ...
    */
    for (u32 i = 1; i <= splits; i++) {
        ...
        frustum_cache[i - 1] = {
            .left = {.point = center - cam_u * radius, .normal = cam_u},
            .right = {.point = center + cam_u * radius, .normal = -cam_u},
            .top = {.point = center + cam_v * radius, .normal = -cam_v},
            .bottom = {.point = center - cam_v * radius, .normal = cam_v},
            .near = {.point = center - cam_front * radius, .normal = cam_front},
            .far = {.point = center + cam_front * radius, .normal = -cam_front},
        };
