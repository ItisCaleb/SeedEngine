...
void EditorCameraBehaviour::start() {
    this->cam = &System::gEngine->get_world()->get_camera();
...
void EditorCameraBehaviour::update(f32 dt) {
    ...
    switch (navigation_mode) {
        ...
        case NavigationMode::Orbit:
            navigation_held = input->is_mouse_pressed(MouseEvent::LEFT);
            break;
        case NavigationMode::None:
            break;
    }
    if (navigation_mode != NavigationMode::None && !navigation_held) {
        end_navigation();
    }
    ...
    if (scroll != 0.0f) {
        if (navigation_mode == NavigationMode::Fly) {
            ...
            orbit_distance =
                std::clamp(orbit_distance * std::pow(kSpeedStep, scroll),
    ...
    Vec2i mouse_position = input->get_mouse_actual_pos();
    const f32 delta_x = (f32)(mouse_position.x - last_mouse_x);
    const f32 delta_y = (f32)(mouse_position.y - last_mouse_y);
    last_mouse_x = mouse_position.x;
    last_mouse_y = mouse_position.y;
